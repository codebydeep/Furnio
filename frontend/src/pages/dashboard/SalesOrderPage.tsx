...
const EMPTY_LINE: Omit<OrderLine, 'subtotal'> = {
  productId: 0, productName: '', quantity: 1, unitPrice: 0, tax: 18,
}

function calcSubtotal(l: Omit<OrderLine, 'subtotal'>) {
  return l.quantity * l.unitPrice
}
function calcTax(l: Omit<OrderLine, 'subtotal'>) {
  return calcSubtotal(l) * l.tax / 100
}

/* ─── Detail Panel ──────────────────────────────────────────── */
function SODetail({
  so, onClose, onCreateInvoice,
}: {
  so: SalesOrder
  onClose: () => void
  onCreateInvoice: (id: number) => Promise<void>
}) {
  const [creating, setCreating] = useState(false)
  const StatusIcon = STATUS_ICON[so.status] ?? Clock

  async function handleInvoice() {
    setCreating(true)
    await onCreateInvoice(so.id)
    setCreating(false)
    onClose()
  }
  ...
  return (
    <Card className="mb-4">
      <CardHeader className="pb-2 flex-row items-center justify-between">
        <CardTitle className="text-sm font-semibold">SO #{so.id} — {so.customerName}</CardTitle>
        <div className="flex items-center gap-2">
          {!so.invoiceId && so.status !== 'cancelled' && (
            <Button size="sm" variant="outline" className="h-7 text-xs gap-1"
              disabled={creating} onClick={handleInvoice}>
              {creating
          ...
          {so.invoiceId && (
            <span className="text-xs text-green-400 flex items-center gap-1">
              <CheckCircle2 size={11} /> Invoice #{so.invoiceId} created
            </span>
          )}
      ...
      <CardContent>
        <div className="grid grid-cols-3 gap-3 mb-4 text-xs">
          <div>
            ...
            <p className="font-medium text-[var(--text)]">{so.customerName}</p>
          </div>
          <div>
            <p className="text-[var(--text-muted)]">Date</p>
            <p className="font-medium text-[var(--text)]">{so.date}</p>
          </div>
          <div>
            <p className="text-[var(--text-muted)]">Status</p>
            <span className={`inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full border ${STATUS_STYLE[so.status]}`}>
              <StatusIcon size={10} /> {so.status}
            </span>
          </div>
        </div>
        <Table>
          ...
          <TableBody>
            ...
            <TableRow className="border-t-2">
              ...
              <TableCell colSpan={2} className="text-right pr-3 text-xs font-semibold">{fmt(so.total)}</TableCell>
            </TableRow>
            <TableRow>
              <TableCell colSpan={4} className="pl-3 text-xs text-[var(--text-muted)]">Tax Total</TableCell>
              <TableCell colSpan={2} className="text-right pr-3 text-xs text-[var(--text-muted)]">{fmt(so.taxTotal)}</TableCell>
            </TableRow>
            <TableRow>
              <TableCell colSpan={4} className="pl-3 text-sm font-bold">Grand Total</TableCell>
              <TableCell colSpan={2} className="text-right pr-3 font-bold text-[var(--accent)] text-base">{fmt(so.grandTotal)}</TableCell>
            </TableRow>
          </TableBody>
        </Table>
      </CardContent>
...
export default function SalesOrderPage() {
  ...
  const taxTotal   = lines.reduce((s, l) => s + calcTax(l), 0)
  ...
  return (
    <div className="db-page">
      <div className="db-page-header">
        <div>
          <h1 className="db-page-title">Sales Orders</h1>
          <p className="db-page-sub">Create sales orders and generate customer invoices.</p>
        </div>
      ...
      {selected && !showForm && (
        <SODetail so={selected} onClose={() => setSelected(null)} onCreateInvoice={handleCreateInvoice} />
      )}

      {showForm && (
        <Card className="mb-4">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-semibold">New Sales Order</CardTitle>
          </CardHeader>
          <CardContent>
            {formErr && (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="auth-field">
                  <label className="auth-label">Customer <span className="text-red-400">*</span></label>
                  <select className="auth-input" value={customerId}
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">SO Lines</p>
                  ...
                  <table className="w-full text-xs">
                    <thead className="bg-[var(--surface-2)]">
                      <tr>
                        ...
                        <th className="text-right px-2 py-2 font-semibold text-[var(--text-muted)] w-24">Base</th>
                        <th className="w-8" />
                      </tr>
                    </thead>
                    <tbody>
                    ...
                    <tfoot className="bg-[var(--surface-2)] border-t border-[var(--border)]">
                      <tr>
                        <td colSpan={4} className="px-3 py-1 text-xs text-[var(--text-muted)]">Subtotal</td>
                        <td className="px-2 py-1 text-right text-xs">{fmt(subTotal)}</td>
                        <td />
                      </tr>
                      <tr>
                        <td colSpan={4} className="px-3 py-1 text-xs text-[var(--text-muted)]">Tax Total</td>
                        <td className="px-2 py-1 text-right text-xs">{fmt(taxTotal)}</td>
                        <td />
                      </tr>
                      <tr>
                        <td colSpan={4} className="px-3 py-2 text-xs font-bold">Grand Total</td>
                        <td className="px-2 py-2 text-right text-sm font-bold text-[var(--accent)]">{fmt(grandTotal)}</td>
                        <td />
                      </tr>
                    </tfoot>
                  </table>
              ...
              <div className="flex justify-end gap-2">
                <Button type="button" variant="outline" size="sm"
                  onClick={() => { setShowForm(false); setCustomerId(0); setLines([{ ...EMPTY_LINE }]) }}>
                  Cancel
                </Button>
                <Button type="submit" size="sm" disabled={saving}>
                  {saving ? <><Loader2 size={13} className="animate-spin mr-1" />Creating…</> : 'Create SO'}
                </Button>
              </div>
            </form>
          </CardContent>
            ...
            <Table>
              ...
              <TableBody>
                {salesOrders.map(so => {
                  const StatusIcon = STATUS_ICON[so.status] ?? Clock
                  return (
                    <TableRow key={so.id} className="cursor-pointer hover:bg-[var(--surface-2)]"
                      onClick={() => { setSelected(so); setShowForm(false) }}>
                      <TableCell className="pl-5 font-mono text-xs text-[var(--text-muted)]">#{so.id}</TableCell>
                      <TableCell className="text-sm font-medium text-[var(--text)]">{so.customerName}</TableCell>
                      <TableCell className="text-xs text-[var(--text-muted)]">{so.date}</TableCell>
                      <TableCell className="text-right text-sm font-semibold">{fmt(so.grandTotal)}</TableCell>
                      <TableCell>
                        <span className={`inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full border ${STATUS_STYLE[so.status]}`}>
                          <StatusIcon size={10} /> {so.status}
                        </span>
                      ...
                      <TableCell>
                        {so.invoiceId
                          ? <span className="text-xs text-green-400">Inv #{so.invoiceId}</span>
